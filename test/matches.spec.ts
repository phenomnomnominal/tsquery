    ...
    describe('tsquery - matches:', () => {
        ...
        it('should find any nodes that implicitly match one of several SyntaxKinds', () => {
            ...
            expect(result).to.deep.equal([
                ...
                ((ast.statements[2] as ExpressionStatement).expression as BinaryExpression).right,
                (((ast.statements[3] as IfStatement).thenStatement as Block).statements[0] as ExpressionStatement).expression
            ]);
        });
    });
});
